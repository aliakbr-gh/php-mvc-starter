<section class="card shadow-sm auth-card">
  <div class="card-body p-4 p-md-5">
    ...
    <form class="vstack gap-3" method="post" action="<?= htmlspecialchars(url('login'), ENT_QUOTES, 'UTF-8') ?>">
        <?= csrf_field() ?>
        <div><label class="form-label" for="email">Email</label><input class="form-control" id="email" type="email" name="email" autocomplete="email" required></div>
        <div><label class="form-label" for="password">Password</label><input class="form-control" id="password" type="password" name="password" autocomplete="current-password" required></div>
