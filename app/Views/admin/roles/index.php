        ...
        <table class="table table-hover align-middle mb-0 data-table">
            ...
            <tbody>
                <?php foreach ($result['items'] as $item): ?>
                    <tr>
                        <td><strong><?= htmlspecialchars($item['name'], ENT_QUOTES, 'UTF-8') ?></strong><small><?= htmlspecialchars($item['slug'], ENT_QUOTES, 'UTF-8') ?></small>
                        </td>
                        ...
                        <td class="actions-cell">
                            <?php if (\App\Core\Auth::can('roles.update')): ?><a class="btn btn-sm btn-outline-primary"
                                    href="<?= htmlspecialchars(url('admin/roles/' . $item['id'] . '/edit'), ENT_QUOTES, 'UTF-8') ?>">Edit</a><?php endif; ?>
